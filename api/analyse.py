...
def get_av_data(ticker):
    ...
    # BUG FIX: PercentInstitutions/PercentInsiders are already percentages (e.g. "65.27" = 65.27%)
    # Use _sf() not _sfpct() to avoid multiplying by 100 and showing 6527%
...
- MINIMUM CONTENT: Each section text field MUST have at least 12 substantive sentences. Do not be brief. Financial professionals reading this expect depth.
...
def _fallback_a(name, sc, fs, lang='en'):
    if lang == 'es':
        return {
            'executive_summary':{'verdict':f'Score {sc["total"]}/100','verdict_sub':'Sintesis AI no disponible.','verdict_color':'amber','verdict_icon':'neutral','text':f'Sintesis completa no disponible. Score compuesto {sc["total"]}/100. Revisa los datos cuantitativos en la barra lateral.'},
...
def analyse(ticker, lang='en'):
    ...
    # Phase 1: ALL data in parallel (Finnhub + AV + Macro together — critical timing fix)
    ...
    # Economic calendar - next 2 weeks of important macro events
    econ_events = []
    try:
        cal_from = time.strftime('%Y-%m-%d', time.gmtime(now_ts))
        cal_to   = time.strftime('%Y-%m-%d', time.gmtime(now_ts + 14*24*3600))
        econ_cal = fh('calendar/economic', {'from': cal_from, 'to': cal_to}, timeout=6)
    ...
    try:
        earn_cal = fh('stock/earnings-calendar', {
            'symbol': ticker,
            'from': to_date,
            'to': time.strftime('%Y-%m-%d', time.gmtime(now_ts + 90*24*3600))
        }, timeout=6)
        if earn_cal.get('earningsCalendar'):
            upcoming_earnings = earn_cal['earningsCalendar'][0].get('date', None)
    except:
        pass

    # Trimmed payload for OpenAI (description capped at 600 chars, hist_fin 3 years, news 4 items)
    user_data_for_ai = {
        'company': {
