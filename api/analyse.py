...
def _repair_json(text):
    """Attempt to fix truncated JSON from OpenAI by closing open braces/brackets."""
    text = text.strip()
    if not text:
        return None
...
def call_openai(system_prompt, user_data, max_tokens=3500):
    try:
        payload = json.dumps({
...
def analyse(ticker, lang='en'):
    ...
    with ThreadPoolExecutor(max_workers=2) as ex:
        fa = ex.submit(call_openai, prompt_a(lang), user_data, 3500)
        fb = ex.submit(call_openai, prompt_b(lang), user_data, 3500)
