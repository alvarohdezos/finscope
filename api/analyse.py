...
# FINscope backend v3.1 — AI section C split into C1 (macro/risk/ownership) + C2 (valuation/competitors/scenarios)
...
def prompt_c2(lang='en'):
    """Section C part 2 — Valuation (§8), Competitors (§9), Scenarios (§10)."""
    lt = _lang_tag(lang)
    return f"""You are the lead equity analyst at FINscope Research. You write INFORMATIONAL REPORTS for portfolio managers and credit analysts (never investment advice). Return ONLY valid JSON. No markdown, no text outside the JSON. Output covers Valuation (§8), Competitors (§9) and Scenarios (§10).
...
def analyse(ticker, lang='en'):
    ...
    try:
        if de is not None and market_cap and op_margin and pe_ttm and pb and float(pb) > 0:
            _book_equity_m = float(market_cap) / float(pb)   # book equity (NOT market cap) is the correct base for D/E
            _debt_m = _book_equity_m * float(de)
            _rev_m = float(market_cap) / float(pe_ttm) / (float(net_margin)/100) if (net_margin and float(net_margin)>0 and float(pe_ttm)>0) else None
            if _rev_m:
    ...
    try:
        _scen = ai.get('scenarios') or {}
        if price and price > 0:
            for _k in ('bull','base','bear'):
                _c = _scen.get(_k)
                if isinstance(_c, dict) and _c.get('price_target'):
                    _ret = (float(_c['price_target']) - price) / price * 100.0
                    if _ret >= 0:
                        _c['upside_pct'] = round(_ret, 1); _c['downside_pct'] = 0
                    else:
                        _c['upside_pct'] = 0; _c['downside_pct'] = round(abs(_ret), 1)
    except Exception:
        pass
